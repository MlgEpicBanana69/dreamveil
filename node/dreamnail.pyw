        ...
        def try_chain_block(self, block, exclusions:list=None):
            ...
            try:
                if self.blockchain.chain_block(block):
                    ...
                    # The commented code adjusts the difficulty target based on the average time a block is recieved.
                    # As of now this feature is disabled due to being proved highly unstable on a small and inactive network.

                    #current_block_time = timeit.default_timer()
                    #self.block_times.append(current_block_time)
                    #wait_parameter = 1 # dreamnail.Server.TRUST_MULTIPLIER
                    #if len(self.block_times) == wait_parameter+1:
                    ...
                    #    dreamnail.singleton.log(f"Recalculated difficulty target. (avg. dt = {result_time / wait_parameter-1})")
